...
pnpm install
pnpm build
pnpm dckl init    # scaffolds .dckl/, CLAUDE.md block, Claude Code skill, heartbeat hook
pnpm dckl         # starts the UI on http://localhost:4321
```

The bundled CLI bundles the server and the pre-built UI assets — a single
`pnpm dckl` boots everything, no orchestration needed.
...
- Terminal 1 runs the bundled CLI, binds to `:4321`, writes `.dckl/.port`.
...
|---|---|
| `pnpm build` | Builds `@dckl/ui` (Vite) and `@dckl/cli` (tsup). UI assets are copied into `packages/cli/dist/ui/`. |
...
| `pnpm dckl` | Starts the CLI (default command: `serve`). |

## dckl subcommands

```bash
pnpm dckl                              # serve — UI + API on :4321
pnpm dckl init                         # scaffold .dckl/ + Claude integration
pnpm dckl status                       # project-wide report
pnpm dckl doctor                       # audit .dckl/ consistency
pnpm dckl task claim <ID>              # mark task as live-worked (amber pulse)
pnpm dckl task release <ID>            # clear the claim
pnpm dckl check <ID> <check-id>        # toggle a reminder or test check
pnpm dckl correction add <ID> "text"   # log an issue found during work
pnpm dckl export <ID>                  # structured Claude prompt for a task
pnpm dckl vision init                  # scaffold .dckl/VISION.md
```
...
```
packages/
  cli/      @dckl/cli    — commander-based CLI, bundles server
  server/   @dckl/server — Hono app factory (source-imported by CLI)
  ui/       @dckl/ui     — React + Vite + Tailwind, built to static assets
```
