/**
 * Content scaffolded by `deckel init` as a first-run tutorial sprint. Three
 * tiny tasks that each exercise one core mechanic:
 *   TSK-01 — check   (click a checkbox, or run `deckel check`)
...
name: Welcome — try Deckel on itself
...
\`deckel init\` scaffolded this sprint as your five-minute tour. Each task
...
1. **Check** — mark an acceptance criterion addressed. Click a checkbox
   in the drawer, or run \`pnpm deckel check <task-id> <check-id>\`.
2. **Claim** — signal that a task is being actively worked on. The UI
   shows an **amber pulse** on the status icon. \`pnpm deckel task claim\`
...
3. **Correction** — log an issue discovered mid-work without leaving the
   CLI. The entry lands in \`.deckel/CHANGELOG.md\` and in the task's
   \`corrections\` field.

### When you're done

    rm -rf .deckel/sprints/sprint-00-welcome
...
test_criteria:
  ...
  - { id: cli-check, label: "Or: run \`pnpm deckel check ${prefix}-01 cli-check\` in a terminal", checked: false }
corrections: []
---
...
2. **In the CLI:** run \`pnpm deckel check ${prefix}-01 <check-id>\`. The
   check ID is the \`id\` field of the entry in \`security_checks\` or
   \`test_criteria\`. The UI picks up the change live via SSE.

Both paths end in the same place: \`.deckel/sprints/sprint-00-welcome/tasks/${prefix}-01.md\`
gets a new \`checked: true\`. Watch the file in git diff if you want to see
the diff-friendliness Deckel is built around.

### What to try

- Flip the \`ui-click\` checkbox above by clicking it.
- Open a terminal and run \`pnpm deckel check ${prefix}-01 cli-check\`.
...
security_checks: []
test_criteria:
  - { id: claim, label: "Run \`pnpm deckel task claim ${prefix}-02\` — status bumps to in_progress, amber pulses on the icon", checked: false }
  - { id: release, label: "Run \`pnpm deckel task release ${prefix}-02\` — amber stops, status stays in_progress", checked: false }
corrections: []
---

## ${prefix}-02 — Claim a task and watch the amber pulse

**Claim** is Deckel's signal for "work is happening here right now". It
...
security_checks: []
test_criteria:
  - { id: correction, label: "Run \`pnpm deckel correction add ${prefix}-03 \\\"Hello from the changelog\\\"\`", checked: false }
...
- The task's \`corrections:\` array (visible in the drawer's Corrections
  section).
- \`.deckel/CHANGELOG.md\` (visible in the Changelog view).
...
1. In a terminal:
   \`pnpm deckel correction add ${prefix}-03 "Hello from the changelog"\`.
