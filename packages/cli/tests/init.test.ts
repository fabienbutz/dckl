...
describe("dckl init", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "dckl-init-"));
    process.exitCode = 0;
  });
  ...
  it("creates the expected .dckl/ layout", async () => {
    await runInit({ cwd: tmp, yes: true, name: "Test Project", prefix: "ENG" });
    expect(existsSync(join(tmp, ".dckl", "config.yaml"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", "sprints"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", "templates", "security-checks.yaml"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", "templates", "test-categories.yaml"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", ".trash"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", ".dcklignore"))).toBe(true);
    expect(existsSync(join(tmp, ".dckl", ".gitignore"))).toBe(true);
  });
  ...
  it("by default scaffolds a 3-task welcome sprint teaching check/claim/correction", async () => {
    ...
    const welcomeDir = join(tmp, ".dckl", "sprints", "sprint-00-welcome");
    ...
    expect(t1).toContain("dckl check TSK-01");
    expect(t2).toContain("dckl task claim TSK-02");
    expect(t2).toContain("dckl task release TSK-02");
    expect(t3).toContain("dckl correction add TSK-03");
  });

  it("--no-demo (noDemo option) skips the welcome sprint", async () => {
    await runInit({ cwd: tmp, yes: true, name: "Test", prefix: "TSK", noDemo: true });
    expect(existsSync(join(tmp, ".dckl", "sprints", "sprint-00-welcome"))).toBe(false);
  ...
  it("welcome sprint has status: active so board is not empty on first run", async () => {
    ...
    const index = readFileSync(
      join(tmp, ".dckl", "sprints", "sprint-00-welcome", "index.md"),
      "utf8",
    );
  ...
  it("writes the chosen project name and prefix into config.yaml", async () => {
    ...
    const cfg = readFileSync(join(tmp, ".dckl", "config.yaml"), "utf8");
    expect(cfg).toContain('name: "LMS for Coaches"');
    expect(cfg).toContain("task_id_prefix: LMS");
  });

  it("refuses to overwrite an existing .dckl/ directory", async () => {
