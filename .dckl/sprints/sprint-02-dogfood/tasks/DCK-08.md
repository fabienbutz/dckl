...
title: 'dckl init writes a real starter sprint, not an empty shell'
type: feature
status: done
security_checks:
...
test_criteria:
  - id: creates-sprint
    label: >-
      `dckl init --yes` in an empty project creates a starter sprint with 3
      demo tasks
    checked: true
  - id: tasks-teachable
    label: >-
  ...
  - id: first-run-no-empty
    label: >-
      `pnpm dckl` right after init shows a populated board, not the 'No
      sprints' empty state
    checked: true
corrections:
  - id: c1
...
## DCK-08: dckl init writes a real starter sprint, not an empty shell

After `dckl init`, the UI is empty. Users don't know what a good task
...
shows that dckl's opinions are worth a first read.
