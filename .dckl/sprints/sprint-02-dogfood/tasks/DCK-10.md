...
title: dckl stop — graceful shutdown of the local server
type: chore
status: done
security_checks:
  - id: input-validation
...
## DCK-10: dckl stop — graceful shutdown

A `pnpm dckl stop` command that reads `.dckl/.port`, signals the
...
- Does not clean up `.port`, so the next `pnpm dckl` walks past the
...
`dckl stop` closes the loop: it's the canonical shutdown command,
uses the `.port` lock as the source of truth for which PID to signal,
and guarantees the next `pnpm dckl` starts on the canonical port.
...
- Auto-idle-shutdown.
- A single-command dev flow (`pnpm dev:all` that boots dckl + vite).
