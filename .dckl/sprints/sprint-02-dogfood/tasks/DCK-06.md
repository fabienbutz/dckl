...
title: dckl doctor — validate .dckl/ consistency
type: feature
status: done
security_checks:
  - id: input-validation
...
test_criteria:
  - id: detects-missing
    label: 'Detects: missing .dckl/, missing config.yaml, malformed task frontmatter'
    checked: true
  - id: detects-orphan
...
corrections:
  ...
  - id: c2
    text: >-
      Multiple stray servers accumulate during dogfood testing — no 'dckl
...
## DCK-06: dckl doctor — validate .dckl/ consistency

A `dckl doctor` command that scans the `.dckl/` tree and reports
inconsistencies. Read-only. Exit codes distinguish clean / warnings /
errors so it can be chained into shell scripts.

### Why

Over time `.dckl/` drifts: orphaned task files (no sprint references
them), corrupted frontmatter from a bad manual edit, stale claims from a
crashed server, missing hooks from a partially-applied `dckl init`.
...
- Auto-fixing problems (reporting only for MVP; `--fix` flag is
  post-MVP).
- Performance benchmarking of the `.dckl/` tree.
- Cross-repo checks.
